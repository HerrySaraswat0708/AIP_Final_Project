...
CLIP produces \emph{uniformly distributed} softmax outputs on all five datasets (Table~\ref{tab:entropy_calibration}).
Its entropy for correct predictions and wrong predictions is virtually identical,
making it impossible to use CLIP's confidence as a reliability signal.

This is a structural consequence of the cosine-similarity scoring with fixed temperature $\tau=100$
applied to a $C$-class distribution: when CLIP text anchors are poorly aligned with test features,
all logits are near-zero and softmax spreads probability uniformly.
...
Spearman rank correlation between entropy and error indicator confirms this quantitatively:
CLIP's $\rho \approx 0$ on all datasets; TDA's $\rho$ ranges from $0.36$ to $0.50$;
FreeTTA's $\rho$ ranges from $0.33$ to $0.59$ (all $p \ll 0.001$).
This is the core mechanism that makes downstream use of their confidence scores meaningful.
Key observations:
\begin{itemize}
