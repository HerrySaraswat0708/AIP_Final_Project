...
- Corr(jsd_clip_fused, correctness) = -0.8561.
- Prediction churn = 0.0000, recovery latency = 0.0000.

## Empirical Link: FreeTTA Internal Signals
- Mean EM confidence-weight vs correctness corr = -0.2419.
- Corr(mu_update_norm, correctness) = -0.2418.
- Mean prototype drift from init moved from ~0.0000 (early) to ~0.0002 (late).
- Prediction churn = 0.0000, recovery latency = 7.0000.

## TDA vs FreeTTA Decision Regions
- Disagreement rate = 90.00%.
- On disagreement samples: FreeTTA win rate = 22.22%, TDA win rate = 0.00%.
