...
- Cache fill ratio rose from ~0.002 (early) to ~0.028 (late), showing growing memory reliance.
- Corr(jsd_clip_fused, correctness) = 0.0731.
- Prediction churn = 1.0000, recovery latency = 7.6667.

## Empirical Link: FreeTTA Internal Signals
- Mean EM confidence-weight vs correctness corr = 0.1865.
- Corr(mu_update_norm, correctness) = 0.1869.
- Mean prototype drift from init moved from ~0.0041 (early) to ~0.0123 (late).
- Prediction churn = 0.0000, recovery latency = 1.4444.

## TDA vs FreeTTA Decision Regions
- Disagreement rate = 83.33%.
- On disagreement samples: FreeTTA win rate = 48.00%, TDA win rate = 0.00%.
